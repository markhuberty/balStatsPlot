...
###############################################
  ...
  balanceMat <- function(cov, bal.out,  means=TRUE){
    ...
    # Check to make sure that the list of covariates is the same length as the list of covariate names

  #  Error <- ifelse(dtype=="matrix"&dim(cov)[2]!=length(bal.out$AfterMatching[]), "Error: Rowcount for the covariate matrix does not equal the covariate count from MatchBalance",
    #       ifelse(dtype=="data.frame"&dim(cov)[2]!=length(bal.out$AfterMatching[]), "Error: Rowcount for the covariate data frame does not equal the covariate count from MatchBalance",
      #            ifelse(dtype=="character"&length(cov)!=length(bal.out$AfterMatching[]) "Error: List of covariate names is not the same length as the list of covariates.", NA)))
    
    #if(Error!=NA) return(Error)
